...
kachok download <file.torrent | magnet:?xt=urn:btih:…> [--dir <path>] [--port <n>]
                ...
                [--high <n>]… [--sequential] [--dht]
                [--encryption plaintext|preferred|required]
```

`--sequential` asks for pieces in order — a piece-length of bytes at each end of every file first,
...
| `swarm/src/main/kotlin/io/github/youndie/kachok/swarm/` | that tracker and that peer, in the module both surfaces test against |
| `cli/src/test/kotlin/io/github/youndie/kachok/cli/CollectorBench.kt` | the collector comparison of research §1.2d, and the only way to redo it |
