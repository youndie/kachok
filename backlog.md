...
| [B-109](docs/backlog/B-109-download-seed-closes-the-set-before-it-seeds.md) `[ ]` | `kachok download --seed` closes the set before it seeds, so nobody can reach it | P2 | S | - |
...
| [B-108](docs/backlog/B-108-an-mcp-server-for-agents.md) `[~]` | An MCP server, so an agent can drive the client | P3 | M | - |
